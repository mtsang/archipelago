...
def multihead_attention(
    ...
    weights = tf.layers.dropout(
        weights, rate=1 - dropout_keep_prob, training=tf.convert_to_tensor(is_training)
    )
...
class AutoInt:
    def __init__(self, args, feature_size, run_cnt):
        ...
        self.feature_size = (
            feature_size  # denote as n, dimension of concatenated features
        )
        self.field_size = args.field_size  # denote as M, number of total feature fields
        self.embedding_size = (
            args.embedding_size
        )  # denote as d, size of the feature embedding
        ...
        self.deep_layers = (
            args.deep_layers
        )  # whether to joint train with deep networks as described in paper
        ...
        with self.graph.as_default():
            ...
            if self.has_wide:
                ...
                self.y_first_order = tf.reduce_sum(
                    tf.multiply(self.y_first_order, feat_value), 1
                )  # None * 1
            ...
            if self.deep_layers != None:
                ...
                for i in range(0, len(self.deep_layers)):
                    ...
                    self.y_dense = tf.nn.dropout(
                        self.y_dense, self.dropout_keep_prob[2]
                    )
                self.y_dense = tf.add(
                    tf.matmul(self.y_dense, self.weights["prediction_dense"]),
            ...
            self.out = tf.add(
                tf.matmul(self.flat, self.weights["prediction"]),
            ...
            self.var1 = [
                v for v in tf.trainable_variables() if v.name != "feature_bias:0"
            ]
            ...
            if self.optimizer_type == "adam":
                if self.has_wide:
                    ...
                    optimizer2 = tf.train.GradientDescentOptimizer(
                        learning_rate=self.learning_rate_wide
                    )
    ...
    def _initialize_weights(self):
        ...
        if self.deep_layers != None:
            ...
            weights["layer_0"] = tf.Variable(
                np.random.normal(
                    loc=0, scale=glorot, size=(layer0_size, self.deep_layers[0])
                ),
                dtype=np.float32,
            )
    ...
    def fit_on_batch(self, Xi, Xv, y):
        ...
        step, loss, opt = self.sess.run(
            (self.global_step, self.loss, self.optimizer), feed_dict=feed_dict
        )
        return step, loss
    ...
    def fit_once(
        ...
        if has_valid:
            ...
            if valid_loss < self.best_loss and self.is_save == True:
                ...
                self.saver.save(
                    self.sess, self.save_path + "model.ckpt", global_step=last_step
                )
                print(
                    "[%d-%d] model saved!. Valid loss is improved from %.4f to %.4f"
    ...
    def predict(self, Xi, Xv):
        ...
        batch_index = 0
        Xi_batch, Xv_batch, y_batch = self.get_batch(
            Xi, Xv, dummy_y, self.batch_size, batch_index
        )
        y_pred = None
        # y_loss = None
