...
class PathExplainerTorch(object):
    ...
    def interactions(
        ...
        if len(input_tensor.shape) != 2:
            raise ValueError(
                "PyTorch Explainer only supports " + "interaction for 2D input tensors!"
            )
        ...
        samples_input, samples_delta = self._get_samples_input(
            input_tensor, baseline, inner_loop_nsamples, use_expectation
        )

        if interaction_index is not None:
            interaction_mult_tensor = torch.zeros(
                [input_tensor.size(0), samples_input.size(1), input_tensor.size(1)]
            )
        else:
            interaction_mult_tensor = torch.zeros(
                [
