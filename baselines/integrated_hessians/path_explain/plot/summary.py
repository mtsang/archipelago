    ...
    for i in range(select_attributions.shape[1]):
        ...
        quant = np.round(
            ...
            * (feature_attr - np.min(feature_attr))
            / (np.max(feature_attr) - np.min(feature_attr) + 1e-8)
        )
...
def summary_plot(
    ...
    **kwargs
    ...
    if standardize_features:
        standardized_feature_values = feature_values - np.mean(
    ...
    feature_df = pd.melt(
        feature_df, var_name="Feature", value_name="Normalized Feature Value"
    )
    attribution_df = pd.melt(
        attribution_df, var_name="Attribution", value_name="Attribution Value"
    )
    attribution_df = attribution_df.drop(columns=["Attribution"])

    jitter_df = _get_jitter_df(
        interactions,
        feature_values,
        select_attributions,
        attributions,
    ...
    x_limits, y_limits = _get_shared_limits(
        melted_df["Attribution Value"], melted_df["Jitter"], scale_x_ind, scale_y_ind
    )
    ...
    for i in range(plot_top_k - 1):
        axis = axs[i]
        _set_axis_config(axis, clear_x_ticks=True, clear_y_ticks=True)
    ...
    axis = axs[-1]
    _set_axis_config(
        axis, [0.0, 0.0, 0.0, 0.5], clear_x_ticks=False, clear_y_ticks=True
    )
    trans = mpl.transforms.blended_transform_factory(axis.transData, axis.transAxes)
    axis.plot(
        [0.0, 1.0],
    ...
    for i in range(plot_top_k):
        ...
        axis.scatter(
            x=selected_df["Attribution Value"],
