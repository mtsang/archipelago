...
def random_word(tokens, tokenizer, mask_token=True):
    """
...
def convert_example_to_features(example, max_seq_length, tokenizer):
    ...
    tokens_a, t1_label = random_word(
        tokens_a, tokenizer, mask_token=False
    )  # do not mask any token
    tokens_b, t2_label = random_word(tokens_b, tokenizer, mask_token=False)
...
def main():
    ...
    parser.add_argument(
        "--do_explain", action="store_true", help="Whether to run explanation"
    )
    parser.add_argument(
        "--train_batch_size",
        default=32,
...
def initialize_prob(model, input_id, segment_id, tokenizer):
    ret = [0]
    for i in range(1, 10):
        input_mask = torch.zeros(1, input_id.size(1)).cuda()
        input_mask[0, i] = 1.0
        masked_input = torch.zeros(1, input_id.size(1)).long().cuda()
        masked_input[0, i] = tokenizer.vocab["[MASK]"]
        pred_scores_a, _ = model(masked_input, segment_id, input_mask)

        pred_scores_a = F.log_softmax(pred_scores_a, -1)[0, i, input_id[0, 1]].item()
        ret.append(pred_scores_a)
    return ret


def mask_and_predict(model, input_id, segment_id, interval_a, interval_b, tokenizer):
    # mask all except interval a, predict interval b
    ...
    pred_scores_a = F.log_softmax(pred_scores_a, -1)
    pred_scores_b = F.log_softmax(pred_scores_b, -1)
...
def build_tree_by_leave_out(
    ...
    for b in range(bs):
        ...
        initial_scores = initialize_prob(model, input_id, segment_id, tokenizer)
        pr_queue = [(i, i + 1, initial_scores[i]) for i in range(1, 10)]
        while len(pr_queue) > 1:
            scores = []
            max_score, max_i = -1000, -1
            for i in range(len(pr_queue) - 1):
                log_prob_a, log_prob_b = mask_and_predict(
                ...
                if prob_incre > max_score:
                    max_score = prob_incre
                    max_i = i
            left, right = pr_queue[max_i][0], pr_queue[max_i + 1][1]
            pr_queue.pop(max_i)
            pr_queue.pop(max_i)
            pr_queue.insert(max_i, (left, right, scores[max_i]))
            print(pr_queue)
...
def predict_and_explain_unbatched(
    ...
    for b in range(bs):
        ...
        while True:
            ...
            for i in range(lm_label_id.size(1)):
                if lm_label_id[0, i].item() != -1:
                    score = rel_logits[0, i, lm_label_id[0, i].item()].item()
                    print("position %d: %f" % (i, score))

            stop_instance = input("next?").strip() == "y"
            if stop_instance:
                print("continuing")
                break
