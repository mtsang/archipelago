...
class YelpProcesser(Sst2Processor):
    pass


class TacredProcessor(Sst2Processor):
    def get_labels(self):
        """See base class."""
        return [str(d) for d in range(42)]
...
def main():
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        output_eval_file = os.path.join(args.output_dir, "eval_results_details.txt")
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(
            ...
            all_pred.extend(pred.reshape(-1).tolist())
            all_truth.extend(label_ids.reshape(-1).tolist())
            ...
            for i in range(input_ids.size(0)):
                logit = logits[i, 1] - logits[i, 0]
                logit = logit.item()
                text = " ".join(
                    tokenizer.convert_ids_to_tokens(input_ids[i].cpu().numpy())
                )
                f.write("%f\t%s\n" % (logit, text))
        ...
        if task_name != "tacred":
            ...
            _, _, eval_accuracy = tacred_f1_score(all_truth, all_pred)
            metric = "f1"
    ...
    for b in range(bs):
        ...
        while True:
            ...
            rel_logits, irrel_logits = model.predict_and_explain(
                input_id, [[(start, stop)]], segment_id, input_mask_single, label_id
            )
            print((rel_logits[0, 1] - rel_logits[0, 0]).item())
...
def predict_and_explain_wrapper_unbatched(
    ...
    for b in range(bs):
        ...
        if normalizer is None:
            return (rel_logits[0, 1] - rel_logits[0, 0]).item()
        else:
            return normalizer(rel_logits, label)
